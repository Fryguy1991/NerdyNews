...
//    private val AppConstants.normalArticle1 = ArticleDisplayModel(
...
//    )
//
//    private val AppConstants.normalArticle2 = ArticleDisplayModel(
...
//    )
    ...
    fun testAttachWithNormalArticle() {
        ...
        if (presenter == null) {
            ...
            presenter.setArticleData(AppConstants.normalArticle1)
            presenter.attach(mockArticleItemView)

            // View should be loaded with data and not closing
            Assert.assertEquals(AppConstants.normalArticle1.sourceName, mockArticleItemView.sourceName)
            Assert.assertEquals(AppConstants.normalArticle1.title, mockArticleItemView.title)
            Assert.assertEquals(AppConstants.normalArticle1.imageUrl, mockArticleItemView.imageUrl)
            Assert.assertEquals(AppConstants.normalArticle1.author, mockArticleItemView.author)
            Assert.assertEquals(AppConstants.normalArticle1.publishedAt, mockArticleItemView.publishedAt)
            Assert.assertEquals(AppConstants.normalArticle1.articleContent, mockArticleItemView.content)
            Assert.assertEquals(AppConstants.normalArticle1.articleUrl, mockArticleItemView.articleUrl)
            Assert.assertFalse(mockArticleItemView.isViewClosing)
        }
    }
    ...
    fun testLoadTwoArticleBeforeAttach() {
        ...
        if (presenter == null) {
            ...
            presenter.setArticleData(AppConstants.normalArticle1)
            presenter.setArticleData(AppConstants.normalArticle2)

            // Attach to view
            presenter.attach(mockArticleItemView)

            // View should be loaded with data from the latest article sent to presenter
            Assert.assertEquals(AppConstants.normalArticle2.sourceName, mockArticleItemView.sourceName)
            Assert.assertEquals(AppConstants.normalArticle2.title, mockArticleItemView.title)
            Assert.assertEquals(AppConstants.normalArticle2.imageUrl, mockArticleItemView.imageUrl)
            Assert.assertEquals(AppConstants.normalArticle2.author, mockArticleItemView.author)
            Assert.assertEquals(AppConstants.normalArticle2.publishedAt, mockArticleItemView.publishedAt)
            Assert.assertEquals(AppConstants.normalArticle2.articleContent, mockArticleItemView.content)
            Assert.assertEquals(AppConstants.normalArticle2.articleUrl, mockArticleItemView.articleUrl)
            Assert.assertFalse(mockArticleItemView.isViewClosing)
        }
    }
